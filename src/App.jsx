  ...
  return (
    <div className="screen active" id="screen-info" style={{ background: 'var(--surface2)', minHeight: '100vh', width: '100%', padding: 16 }}>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        ...
        <div style={{ borderTop: '0.5px solid var(--border-subtle)', marginTop: 8, paddingTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--color-text-primary)' }}>中包袋数 / 중간포장 수</span>
          <span style={{ fontSize: 13, color: 'var(--color-text-primary)' }}>
            {moData?.inner_pack_count != null ? String(moData.inner_pack_count) + ' 个' : '미생성 / 未生成'}
          </span>
        </div>
        <div style={{ borderTop: '0.5px solid var(--border-subtle)', marginTop: 8, paddingTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--color-text-primary)' }}>麻袋数 / 마대 수</span>
          <span style={{ fontSize: 13, color: 'var(--color-text-primary)' }}>
            {moData?.master_bag_count != null ? String(moData.master_bag_count) + ' 个' : '미생성 / 未生成'}
          </span>
        </div>
      </div>
...
// Strip CJK (Chinese) characters from a color label like "黑色 BLACK" → "BLACK".
// Handles comma-joined values by processing each token, then collapses spaces.
function stripCjk(str) {
  return String(str ?? '')
    .split(',')
    .map(tok => tok.replace(/[一-鿿　-〿＀-￯]+/g, '').replace(/\s+/g, ' ').trim())
    .filter(Boolean)
    .join(', ');
}
...
const BagCreateQtyScreen = memo(function BagCreateQtyScreen({
  ...
  qty, setQty, size, setSize, colors, setColors, unit, setUnit,
            ...
            <DkCard>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 0', borderBottom:'1px solid var(--app-divider)' }}>
                ...
                <select
                  ...
                  style={{ fontSize:10, flexShrink:0, border:'1px solid '+G.border, borderRadius:4, padding:'2px 4px', background:G.card, color:G.cream, cursor:'pointer', fontFamily:'inherit', outline:'none' }}
                >
                  <option value="套/set">套/set</option>
                  <option value="件/pcs">件/pcs</option>
                </select>
              </div>
              <div style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 0', borderBottom:'1px solid var(--app-divider)' }}>
    ...
    <DkScreen style={{ paddingTop:0 }}>
      ...
      <div style={{ padding:'20px 20px 40px' }}>
        ...
        <DkCard>
          ...
          <div style={{ fontSize:12, color:G.cream, marginBottom:4 }}>{bags.length}개 마대 · {bag.totalQty} 件 总计</div>
          {(() => {
            const stdCount = bags.filter(b => !b.isRemainder).length;
            const remCount = bags.filter(b => b.isRemainder).length;
            return (
              <div style={{ fontSize:11, color:G.goldDim, marginBottom:8 }}>
                표준 {stdCount}개 + 자투리 {remCount}개 = 총 {bags.length}개
              </div>
            );
          })()}
          {bags.map((b, i) => (
    ...
    <DkScreen style={{ paddingTop:0 }}>
      ...
      <div style={{ padding:'20px 20px 40px' }}>
        <DkCard>
          ...
          {bagRecord.notes_colors != null ? (
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 0', borderBottom:'1px solid var(--app-divider)' }}>
              ...
              <span style={{ fontSize:12, color:G.cream, textAlign:'right', display:'flex', flexWrap:'wrap', justifyContent:'flex-end', gap:6 }}>
                {stripCjk(bagRecord.notes_colors).split(',').map(c => c.trim()).filter(Boolean).map((c, i) => (
          ...
          <DkRow label="数量 / 수량" value={bagRecord.notes_qty != null ? bagRecord.notes_qty + ' ' + bagRecord.notes_unit : '-'} />
...
export default function App() {
  ...
  const [bagUnit, setBagUnit] = useState('套/set');
  ...
  const fetchMOData = useCallback(async (moNumber, sku, factory) => {
    ...
    try {
      ...
      const next = {
        ...
        inner_pack_count: (r['Inner_Pack_Count'] != null && r['Inner_Pack_Count'] !== '') ? r['Inner_Pack_Count'] : null,
        master_bag_count: (r['Master_Bag_Count'] != null && r['Master_Bag_Count'] !== '') ? r['Master_Bag_Count'] : null,
      };
      setMoData(next);
      setMoRecordId(r['ID']);
    ...
    try {
      ...
      (async () => {
        try {
          ...
          // Unassigned leftover packs (Is_Remainder == true && !Assigned_To_Bag)
  ...
  const handleCreateBagQty = useCallback(async () => {
    ...
    const bagData = {
      ...
      'Bag_Status':       'Created',
      'Notes':            JSON.stringify({ qty: bagQty, size: bagSize.trim(), colors: bagColors.trim(), unit: bagUnit }),
    };
  ...
  }, [bagMO, bagSelectedLeftoverUuids, bagLeftoverPacks, bagWorker, bagQty, bagSize, bagColors, bagUnit]);
